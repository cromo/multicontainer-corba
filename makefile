...
iiopnet-ping-server:
	docker build -f Dockerfile-iiopnet-ping-server -t iiopnet-ping-server .
...
orbit-ping-server:
	docker build -f Dockerfile-orbit-ping-server -t orbit-ping-server .

orbit-ping-client:
	docker build -f Dockerfile-orbit-ping-client -t orbit-ping-client .
...
run-iiopnet-ping-server: iiopnet-ping-server
	docker run -it --rm --net=corba iiopnet-ping-server
...
run-orbit-ping-server: orbit-ping-server
	docker run -it --rm --net=corba orbit-ping-server

run-orbit-ping-client: orbit-ping-client
	docker run -it --rm --net=corba orbit-ping-client
